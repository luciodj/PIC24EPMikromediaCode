...
/**
 *  connect the output compare module #1 to the RD2 pin
 */
void BacklightInit()
{
    ...
    PPSOutput(OUT_FN_PPS_OC1, OUT_PIN_PPS_RP66);   // OC1 = RP66 D2/pin 66
...
WORD GOLMsgCallback(WORD objMsg, OBJ_HEADER *pObj, GOL_MSG *pMsg)
{
    ...
    if( pObj->ID == SLD_2 )
    {
        // obtain pointer to Digital meter
        pDM = (DIGITALMETER*) GOLFindObject( DMT_3);

        // read slider position and update the meter
        DmSetValue( pDM, SldGetPos( pObj));

        // force the digital meter to update image on screen
        SetState( pDM, DM_UPDATE);

        // update the screen backlight setting
        BacklightSet( SldGetPos( pObj));
    }

#ifdef _SCREENCAPTURE
    else if ( pObj->ID == WIN_1)
    {
        FSInit();
        ScreenCapture( "GDDSlide.scr");
    }
#endif
...
// Set the other chip selects to a known state
    ...
    // set the peripheral pin select for the PSI channel used
        ...
        RPOR11bits.RP23R = 18;                  // connect pin RD2 to OC1 output
    ...
    // configure OC1 to generate a PWM signal and control the backlight
    BacklightInit();
