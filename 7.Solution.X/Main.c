...
/* globals */
WORD TMRX=0, TMRXflag=0;
#define TMRX_PERIOD     2000
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if ( TMRXflag)
        {
            ...
            s = (PROGRESSBAR*)GOLFindObject( PRB_X);
            PbSetPos( s, (WORD)  ((( x>>1) +128) * 100) >> 8);
            SetState( s, PB_DRAW_BAR);

            s = (PROGRESSBAR*)GOLFindObject( PRB_Y);
            PbSetPos( s, (WORD)  ((( y>>1) +128) * 100) >> 8);
